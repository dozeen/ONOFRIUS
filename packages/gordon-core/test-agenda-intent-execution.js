/**
 * test-agenda-intent-execution.js - Test di verifica dell'Esecuzione Deterministica degli Intent Agenda e del Guardiano Anti-Allucinazione
 */

const AgendaEngine = require("./agenda/AgendaEngine");
const AgendaCapability = require("./capability/AgendaCapability");
const CapabilityRouter = require("./capability/CapabilityRouter");
...
async function runTest() {
    ...
    // TEST 1: Intercettazione Query Agenda (Zero LLM Tokens, Zero Allucinazioni)
    console.log("--- TEST 1: Intercettazione Query Agenda ---");
    ...
    // TEST 2: Aggiunta Appuntamenti Reali ed Estrazione Deterministica
    console.log("--- TEST 2: Estrazione Appuntamenti Reali dell'Agenda ---");
    AgendaEngine.add({
    ...
    if (res2.handled && ctx2.response.includes("Riunione con Mario TEST")) {
        console.log("✅ TEST 2 PASSED: Elenco deterministico dell'agenda formattato correttamente con i dati reali.\n");
    } else {
        console.error("❌ TEST 2 FAILED!");
        process.exit(1);
    }

    // TEST 3: Intercettazione Allucinazione Operativa nel FactVerifier
    console.log("--- TEST 3: Blocco Allucinazione Operativa ('Controlla l'app sul PC') ---");
